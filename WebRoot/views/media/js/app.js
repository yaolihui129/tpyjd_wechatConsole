...
var App = function () {
	...
	var handlePassChange = function(){
		$("#submitBtn").click(function(e) {
			...
			if(flag){
				//校验原密码是否正确
				$.post("/valiOldPassword",{"oldpass":pass0},function(data){
					if(data.status != 0){
						alert('原密码不正确！');
						$("#password0").val("");
						$("#password0").focus();
						...
						if(flag && (pass1.length < 6 || pass1.length > 20)){
							alert("新密码长度为6到20位!");
							flag = false;
						}
						...
						if(flag){
							$.post("/password",{"password":pass1,"oldpass":pass0},function(data){
								if(data.status==0){
									alert('密码修改成功！');
    ...
    return {
        ...
        init: function () {
			...
			handlePassChange();//change password 
