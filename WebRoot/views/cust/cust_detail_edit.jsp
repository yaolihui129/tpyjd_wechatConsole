					...
					<!--  --><div class="span12">
						<div class="tabbable tabbable-custom boxless">
							<ul class="nav nav-tabs">
								...
								<li id="li_tab4" <c:if test="${active == '3' }">class="active"</c:if>><a href="#tab_4" data-toggle="tab">签约信息</a></li>
							</ul>
							...
							<div class="tab-content">
													...
													<div class="span6 ">
														<div class="control-group">
															...
															<c:forEach items="${countryList}" var="item">
																	<c:if test="${intention.country ==  item.dictCode}">
														...
														<div class="control-group">
															...
															<c:forEach items="${visaTypeList}" var="visaType">
																	<c:if test="${intention.identity_type ==  visaType.dictCode}">
																		<option value="${visaType.dictCode }" selected>${visaType.name}</option>
																	</c:if>
																	<c:if test="${intention.identity_type !=  visaType.dictCode}">
																		<option value="${visaType.dictCode }">${visaType.name}</option>
																	</c:if>
														...
														<div class="control-group">
															...
															<c:forEach items="${languageList}" var="lan">
																	<c:if test="${intention.language ==  lan.dictCode}">
																		<option value="${lan.dictCode }" selected>${lan.name}</option>
																	</c:if>
																	<c:if test="${intention.language !=  lan.dictCode}">
																		<option value="${lan.dictCode }">${lan.name}</option>
																	</c:if>
														...
														<div class="control-group">
															...
															<c:forEach items="${liveList}" var="live">
																	<c:if test="${intention.live_require ==  live.dictCode}">
																		<option value="${live.dictCode }" selected>${live.name}</option>
																	</c:if>
																	<c:if test="${intention.live_require !=  live.dictCode}">
																		<option value="${live.dictCode }">${live.name}</option>
																	</c:if>
														...
														<div class="control-group">
															...
															<c:forEach items="${ageList}" var="age">
																	<c:if test="${intention.custSource ==  age.dictCode}">
																		<option value="${age.dictCode }" selected>${age.name}</option>
																	</c:if>
																	<c:if test="${intention.custSource !=  age.dictCode}">
																		<option value="${age.dictCode }">${age.name}</option>
																	</c:if>
														...
														<div class="control-group">
															...
															<c:forEach items="${manageList}" var="manage">
																	<c:if test="${intention.custSource ==  manage.dictCode}">
																		<option value="${manage.dictCode }" selected>${manage.name}</option>
																	</c:if>
																	<c:if test="${intention.custSource !=  manage.dictCode}">
																		<option value="${manage.dictCode }">${manage.name}</option>
																	</c:if>
								...
								<div class="tab-pane " id="tab_4">
									<c:if test="${empty signedList || fn:length(signedList) == 0 }">
										<!-- BEGIN EXAMPLE TABLE PORTLET-->
									<div class="portlet box green">
										<div class="portlet-title">
									...
									<c:if test="${not empty signedList && fn:length(signedList) != 0}">
									<a href="/cust/signrecord/add/${custId }" class="btn blue addnew"><i class=" icon-plus"></i> 新增</a>
											...
											<div class="row-fluid">
												...
												<div class="span6 ">
													<div class="control-group">
														...
														<fmt:formatDate value="${signed.sign_date }" pattern="yyyy-MM-dd"/>
													</div>
												</div>
												<!--/span-->
											</div>
				...
				<form id="addRecord" action="/cust/followrecord/add" method="post">
					<table>
						...
						<tr>
							...
							<input type="text" id="start_time" name="start_time" class="laydate-icon" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
							<script>
							laydate({
						...
						<tr>
							<td>结束时间</td>
							<td>
							<input type="text" id="end_time" name="end_time" class="laydate-icon" onclick="laydate({istime: true, format: 'YYYY-MM-DD hh:mm:ss'})">
							<script>
							laydate({
				...
				.ready(
						function() {
							...
							$("#intentionBtn")
									.click(
											function(e) {
												var fromurl = window.location.href; //document.referrer;
												$("#fromurl").val(fromurl);
