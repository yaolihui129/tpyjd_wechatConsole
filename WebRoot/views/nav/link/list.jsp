...
<body class="page-header-fixed">
			...
			<div class="container-fluid">
				...
				<div class="portlet box grey">
							<div class="portlet-title">
								<div class="actions">
                                    <a href="/official/nav/link/add" class="btn blue"><i class="icon-pencil"></i>新增友情链接</a>
                                </div>
							</div>
											...
											<c:forEach var="item" items="${list}" varStatus="status">
												<tr>
													...
													<td>
														...
														<a  href="/official/nav/link/updata/${item.friendly_link_id}" class="btn green-stripe mini edit"><i class="icon-edit"></i> 编辑</a>
	...
	<script>
		...
		function del(id) {
			if(confirm("确定要删除吗？")){
				$.post("/official/nav/link/delete/"+id, function(data){
					if(data.status==0) {
						alert(data.msg);
		...
		function orderUpdata(id, order) {
			$.post("/official/nav/link/order", {'id':id, 'order':order}, function(data){
				if(data.status==0) {
					/* alert(data.msg); */
					window.location.reload();
				} else {
