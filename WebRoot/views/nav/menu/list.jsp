...
<body class="page-header-fixed">
			...
			<div class="container-fluid">
				...
				<div class="portlet box grey">
							<div class="portlet-title">
								<div class="actions">
                                    <a href="/official/nav/menu/add" class="btn blue"><i class="icon-pencil"></i>新增</a>
                                </div>
							</div>
											...
											<c:forEach var="item" items="${list}" varStatus="status">
												<tr>
													...
													<td>
														...
														<a  href="/official/nav/menu/updata/${item.nav_menu_id}" class="btn green-stripe mini edit"><i class="icon-edit"></i> 编辑</a>
														<c:choose>
															<c:when test="${menu_status_yfb==item.menu_status}">
																<a href="javascript:void(0);" class="btn red-stripe mini" onclick="updataStatus('${item.nav_menu_id}', '${menu_status_wfb}','下线成功');"><i class="icon-minus"></i>下线</a>
															</c:when>
															<c:when test="${menu_status_wfb==item.menu_status}">
																<a href="javascript:void(0);" class="btn red-stripe mini" onclick="updataStatus('${item.nav_menu_id}', '${menu_status_yfb}','发布成功');"><i class="icon-minus"></i>发布</a>
															</c:when>
														</c:choose>
														<a href="/official/nav/menu/two/${item.nav_menu_id}" class="btn blue-stripe mini"><i class="icon-sitemap"></i>子菜单</a>
													</td>
												</tr>
											</c:forEach>
										</c:if>
	...
	<script>
		...
		/* //删除
		function del(id) {
			$.post("/official/nav/menu/delete/"+id, function(data){
				if(data.status==0) {
					alert(data.msg);
		...
		function orderUpdata(id, order) {
			$.post("/official/nav/menu/order", {'id':id, 'order':order}, function(data){
				if(data.status==0) {
					/* alert(data.msg); */
					window.location.reload();
				} else {
		...
		//状态修改
		function updataStatus(id, code, msg) {
			$.post("/official/nav/menu/release",{'id':id, 'code':code}, function(data){
				if(data.status==0) {
					alert(msg);
					window.location.reload();
				} else {
