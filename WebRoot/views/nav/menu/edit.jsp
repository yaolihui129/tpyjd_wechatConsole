                                ...
                                <div class="tab-pane active">
                                    <div class="portlet-body form">
                                        ...
                                        <form id="sform" class="form-horizontal"  method="post">
                                            ...
                                            <div class="row-fluid">
                                                <div class="span12 ">
                                                    <div class="control-group">
                                                        ...
                                                        <div class="controls">
                                                            <input type="text" class="m-wrap span5" data-length="10" must-be="true" data-label="一级菜单" 
                                                              placeholder="一级菜单" name="menu_name" value="${menu.menuName}">
                                            ...
                                            <div class="row-fluid">
                                                <div class="span12 ">
                                                    <div class="control-group">
                                                        ...
                                                        <div class="controls">
                                                            <input type="text" class="m-wrap span5" data-length="200" must-be="true" data-label="链接地址" 
                                                              placeholder="链接地址" name="menu_url" value="${menu.menuUrl}">
                                                              <span style="color:red;margin-left:10px;">*</span>
                                                        </div>
                                                    </div>
                                                </div>
                                                     ...
                                                     <div class="control-group">
														<label class="control-label">导航类型:</label>
														 <div class="controls" >
															...
															<c:choose>
																<c:when test="${menu != null and menu.menuType != null}">
																	<c:if test="${null != typeList}">
																		<c:forEach var="item" items="${typeList}">
																				<c:if test="${menu.menuType==item.dictCode}">
																					<p style="line-height: 35px;">${item.name}</p>
																				</c:if>
																		</c:forEach>
																	</c:if>
																</c:when>
																 <c:otherwise>
																 	<c:if test="${null != typeList}">
																		<c:forEach var="item" items="${typeList}" varStatus="status">
																			<label class="radio" >
																				<input type="radio" name="menu_type" value="${item.dictCode}" 
																				<c:if test="${checked != null || status.count==1}">checked="checked"</c:if>/>${item.name}
																			</label>
																		</c:forEach>
																	</c:if>
                                            ...
                                            <!--/row-->
											<input  name="friendly_link_id" type="hidden" value="${menu.navMenuId}" />
