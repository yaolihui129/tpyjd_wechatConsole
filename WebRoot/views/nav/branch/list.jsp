...
<body class="page-header-fixed">
			...
			<div class="container-fluid">
				...
				<div class="portlet box grey">
							<div class="portlet-title">
								<div class="actions">
                                    <a href="/official/nav/branch/add" class="btn blue"><i class="icon-pencil"></i>新增分公司</a>
                                </div>
							</div>
												...
												<tr>
													...
													<td>
														<%-- <c:choose>
		                                            		...
		                                            		<c:otherwise>
																<a href="javascript:;" onclick="orderUpdata('${item.friendly_link_id}', 'up');" class="btn black-stripe mini"><i class="icon-arrow-up"></i> 上移</a>
		                                            		</c:otherwise>
		                                            	</c:choose>
		                                            	<c:choose>
		                                            		<c:when test="${now==page and status.count==fn:length(list)}"></c:when>
		                                            		<c:otherwise>
		                                            			<a href="javascript:;" onclick="orderUpdata('${item.friendly_link_id}', 'down');" class="btn black-stripe mini"><i class="icon-arrow-down"></i> 下移</a>
		                                            		</c:otherwise>
		                                            	</c:choose> --%>
														<a  href="/official/nav/branch/updata/${item.friendly_link_id}" class="btn green-stripe mini edit"><i class="icon-edit"></i> 编辑</a>
														<a href="javascript:void(0);" class="btn red-stripe mini" onclick="del('${item.friendly_link_id}');"><i class="icon-trash"></i> 删除</a>
													</td>
												</tr>
											</c:forEach>
										</c:if>
	...
	<script>
		...
		function del(id) {
			if(confirm("确定要删除吗？")){
				$.post("/official/nav/branch/delete/"+id, function(data){
					if(data.status==0) {
						alert(data.msg);
		...
		function orderUpdata(id, order) {
			$.post("/official/nav/branch/order", {'id':id, 'order':order}, function(data){
				if(data.status==0) {
					alert(data.msg);
					window.location.reload();
				} else {
