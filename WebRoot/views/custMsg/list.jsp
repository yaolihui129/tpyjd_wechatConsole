...
<body class="page-header-fixed">
			...
			<div class="container-fluid">
				...
				<div class="portlet box grey">
							<div class="portlet-title">
								<div class="actions" style="height:26px;">
                                </div>
							</div>
										...
										<c:if test="${list != null}">
											<c:forEach var="item" items="${list}" varStatus="status">
												<tr>
													...
													<td>
														<c:choose>
															<c:when test="${item.url != null and !empty item.url}"><a href="${OFFICIAL_URL}${item.url}" target="_blank">${item.source}</a></c:when>
													...
													<td>${item.remark}</td>
													<td>
														<a  href="/data/custMsg/updata/${item.custinfo_message_id}" class="btn green-stripe mini edit"><i class="icon-edit"></i> 编辑</a>
														<a href="javascript:void(0);" class="btn red-stripe mini" onclick="del('${item.custinfo_message_id}');"><i class="icon-trash"></i> 删除</a>
													</td>
												</tr>
											</c:forEach>
										</c:if>
	...
	<script>
		...
		function del(id) {
			if(confirm("确定要删除吗？")){
				$.post("/data/custMsg/delete/"+id, function(data){
					if(data.status==0) {
						alert(data.msg);
