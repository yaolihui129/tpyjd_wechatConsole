                    ...
                    <div class="portlet box grey">
							...
							<div class="portlet-title">
								...
								<div class="actions">
									...
									<a href="/cms/imageinfo/add" class="btn blue addnew"><i class="icon-pencil"></i> 新增</a>
                   ...
                   var checks = $("input.checkboxes:checked");
					if(checks.size()>0){
						...
						if(confirm("提示：数据一旦删除不能恢复!确定要删除吗?")){
							$.post("/cms/imageinfo/delete",{"ids":ids},function(data){
								if(data.status==0){
									checks.each(function(index, element) {
                   ...
                   var checks = $("input.checkboxes:checked");
					if(checks.size()>0){
						...
						if(confirm("确定要发布选中的资讯信息吗?")){
							$.post("/cms/imageinfo/publish",{"ids":ids},function(data){
								if(data.status==0){
									//checks.each(function(index, element) {
										alert(data.msg);
										location.reload();
			...
			//绑定删除按钮
				$("i.icon-trash").each(function(index, element) {
					$(this).click(function(e){
						var id = $(this).parent().attr("data-id");
						if(confirm("提示：数据一旦删除不能恢复!确定要删除吗?")){
							$.post("/cms/imageinfo/delete",{"ids":id},function(data){
								if(data.status==0){
				...
				$("i.icon-remove").each(function(index, element) {
					$(this).click(function(e){
						var id = $(this).parent().attr("data-id");
						if(confirm("确定要下线吗?")){
							$.post("/cms/imageinfo/offline",{"ids":id},function(data){
								if(data.status==0){
									alert("下线成功!");
									location.reload();
								}
