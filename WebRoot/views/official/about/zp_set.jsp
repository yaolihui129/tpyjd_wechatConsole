								...
								<form action="#" class="form-horizontal" id="articleForm" method="post">
									...
									<div class="control-group ">
										...
										<div class="controls">
											...
											<textarea class="span9 m-wrap" rows="5" id="contents" maxlength="200" name="contents" must-be="true" data-label="内容">${config.value}</textarea>
										</div>
									</div>
									...
									<div class="form-actions">
										...
										<button type="button" class="btn blue" id="saveBtn"><i class="icon-ok"></i> 保存并发布</button>
			...
			$("#saveBtn")
					.click(
							function(e) {
								...
								/* 
								if($("#contents").val() == "${config.value}"){
									alert("内容没有改变，无需保存");
									return;
								}  */
								
								$.post("/official/about/zp/saveConfig/${config.ocId}", { "contents": $('#contents').val()},
										function(data){
											if(data.status==0){
												alert(data.msg);
												location.href = "/official/about/zp";
											}
											else {
												alert(data.msg);
												return false;
