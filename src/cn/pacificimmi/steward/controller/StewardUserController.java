...
public class StewardUserController extends Controller {
	public void index(){
		 ...
		 * 渲染视图
		 */
		this.renderJsp("/views/steward/steward_user.jsp");
	}
	...
	public void tryAdd(){ 
		...
		if(!sp.getIds().isEmpty()){
			...
			for (int i = 0; i < split.length; i++) {
				...
				if(null!=user){
					StewardUser su = StewardUser.dao.findFirst("select * from crm_steward_user where user_id="+split[i]);
					if(su==null){
						su=new StewardUser(); 
...
//			this.addWhereSegmentByAnd("u.delete_flag='0' and u.crm_id<>null and login_name<>'admin' AND  u.user_id NOT IN(SELECT user_id FROM crm_steward_user where delete_flag='0')");
			this.addWhereSegmentByAnd("u.delete_flag='0' and login_name<>'admin' AND  u.user_id NOT IN(SELECT user_id FROM crm_steward_user where delete_flag='0')");	
			//查询所属公司
	...
	public void remove(){
		...
		Map<String,Object> rst = new HashMap<String,Object>();
		if(!sp.getIds().isEmpty()){
			Db.update("update console_user set delete_flag='INVALID' where user_id in ("+sp.getIds()+")");
			rst.put("status", 0);
			rst.put("msg", "删除成功");
			this.renderJson(rst);		
		}
